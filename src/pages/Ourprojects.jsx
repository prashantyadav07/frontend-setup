...
const IntroHeader = () => {
  ...
  return (
    <header ref={headerRef} className="text-center md:text-left mb-24 pt-10">
      <motion.div
        initial={{ width: 0 }}
        animate={isInView ? { width: '120px' } : {}}
        transition={{ duration: 1, delay: 0.2, ease: premiumEase }}
        className="h-[2px] bg-[#B4A388] mb-8 mx-auto md:mx-0"
      />
      <h2 className="text-5xl md:text-7xl lg:text-8xl font-bold text-[#111] tracking-tight flex flex-wrap justify-center md:justify-start gap-4">
        {words.map((word, i) => (
          <motion.span
            key={i}
      ...
      <motion.p
        ...
        className="text-xl md:text-2xl text-gray-500 mt-6 max-w-2xl mx-auto md:mx-0 font-light"
      >
        ...
        <div className="relative bg-white shadow-md shadow-black/5 p-1.5 rounded-full flex border border-gray-100">
          {['Ongoing', 'Completed'].map((tab) => (
            <button
              ...
              className={`relative z-10 px-10 py-3.5 rounded-full text-sm font-semibold tracking-wider uppercase transition-colors duration-300 ${
                active === tab ? 'text-white' : 'text-gray-500 hover:text-[#111]'
              }`}
            >
              {active === tab && (
                <motion.div
